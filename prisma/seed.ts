...
async function main() {
  ...
  // SPEC
  const specializations = ['C', 'AIA', 'ETTI', 'EN', 'SE', 'ESM'];

  for (let i = 0; i < specializations.length; i++) {
    await prisma.specialization.create({
      data: {
        level: specializations[i], // Use specialization name from the array
      },
    });
  }
   ...
   for (let i = 1; i <= 10; i++) {
    await prisma.exam.create({
      data: {
        ...
        lessonId: (i % 30) + 1,
        roomId: (i % rooms.length) + 1, // Assigning rooms to exams
      },
    });
  }
